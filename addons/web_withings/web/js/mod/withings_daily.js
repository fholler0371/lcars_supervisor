define(['lcsmdefaultpanel'], function() {
    window.modul['withings_daily'] = {
        modul: true,
        icon: '/img/mdi/badge-account-outline.svg',
        label: 'tägliche Aktivität',
        already_init : false,
        panel:'withings_daily',
        init: function () {
          let _default_panel_ = window.modul['__default_panel__']
          let self = window.modul['withings_daily']
          self.history_click = _default_panel_.history_click
          self.history_create = _default_panel_.history_create
        },
        set_content: function() {
            let self = window.modul['withings_daily']
            if (!($('#content_modul_page_withings_daily').length)) {
                let html = '<div id="content_modul_page_withings_daily" class="content_modul"></div>'
                $(".main_content").append(html)
                require([], function() {
                    window.api_call(url='sm/get_cards', data={'panel': 'daily'}).then(resp => {
                        if (resp.ok) {
                            ...
                            require(req, function(Packery){
                                for (var j=0; j<items.length; j++) {
                                    ...
                                    cards[type].create('withings_daily', items[j]['label'], j, item_data)
                                }
                                $('#content_modul_page_withings_daily').data('packery', new Packery( '#content_modul_page_withings_daily', {itemSelector: '.sh_card', gutter: 20}))
                            })
                            console.log(resp)
                        }
                    })
            ...
            $('#content_modul_page_withings_daily').show()
        },
        show: function () {
            let self = window.modul['withings_daily']
            window.modul['helper'].activate('withings_daily', self.label)
            self.set_content()
        },
        stop: function() {
    ...
    window.modul['withings_daily'].init()
});
