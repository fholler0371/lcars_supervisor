define(['lcsmdefaultpanel'], function() {
    window.modul['withings_sleep'] = {
        ...
        init: function () {
          ...
          let self = window.modul['withings_sleep']
          self.history_click = _default_panel_.history_click
          self.history_create = _default_panel_.history_create
        },
        set_content: function() {
            let self = window.modul['withings_sleep']
            if (!($('#content_modul_page_withings_sleep').length)) {
                let html = '<div id="content_modul_page_withings_sleep" class="content_modul"></div>'
                $(".main_content").append(html)
                require([], function() {
                    window.api_call(url='sm/get_cards', data={'panel': 'sleep'}).then(resp => {
                        if (resp.ok) {
                            ...
                            require(req, function(Packery){
                                for (var j=0; j<items.length; j++) {
                                    ...
                                    cards[type].create('withings_sleep', items[j]['label'], j, item_data)
                                }
                                $('#content_modul_page_withings_sleep').data('packery', new Packery( '#content_modul_page_withings_sleep', {itemSelector: '.sh_card', gutter: 20}))
                            })
                            console.log(resp)
                        }
                    })
            ...
            $('#content_modul_page_withings_sleep').show()
        },
        show: function () {
            let self = window.modul['withings_sleep']
            window.modul['helper'].activate('withings_sleep', self.label)
            self.set_content()
        },
        stop: function() {
    ...
    window.modul['withings_sleep'].init()
});
