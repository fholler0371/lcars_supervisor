define(function () {
  var card = {
    create : function(panel, label, id, params) {
      ...
      let _items = ['snoring', 'episoden']
      var html = '<div class="sh_card" data-panel="'+panel+'" data-id="'+id+'" data-type="withings_sleep_snoring"><div class="sh_card_label">'+label+'</div></div>'
      ...
      window.api_call(url='sm/cards_source', data={'panel': panel, 'card': id,
        ...
        if (resp.ok) {
          ...
          cards.withings_sleep_snoring.update(panel, id, values)
    ...
    update : function(panel, id, values) {
      ...
      for (var i=0; i<eles.length; i++) {
        var ele = eles[i]
        if (($(ele).data('entry') == 'snoring') && (values['snoring'] != null)) {
          var div_ele = $(ele).find('.sh_card_item_value'),
              span_ele = $(div_ele[0]).find('span'),
              old_text = $(span_ele[0]).text(),
              new_text = (values['snoring'] / 60).toLocaleString('de-DE', {minimumFractionDigits: 0, maximumFractionDigits: 0})
          //if (old_text != new_text) { $(div_ele[0]).addClass('sh_card_item_highlight') }
          $(span_ele[0]).text(new_text)
        }
        if (($(ele).data('entry') == 'episoden') && (values['episoden'] != null)) {
          var div_ele = $(ele).find('.sh_card_item_value'),
              span_ele = $(div_ele[0]).find('span'),
              old_text = $(span_ele[0]).text(),
              new_text = values['episoden'].toLocaleString('de-DE', {minimumFractionDigits: 0, maximumFractionDigits: 0})
          //if (old_text != new_text) { $(div_ele[0]).addClass('sh_card_item_highlight') }
          $(span_ele[0]).text(new_text)
        }
      }
  ...
  window.cards['withings_sleep_snoring'] = card
})