define(['lcsmdefaultpanel'], function() {
    window.modul['withings_heart'] = {
        modul: true,
        icon: '/img/mdi/heart.svg',
        label: 'Herz',
        already_init : false,
        panel:'withings_heart',
        init: function () {
          let _default_panel_ = window.modul['__default_panel__']
          let self = window.modul['withings_heart']
          self.history_click = _default_panel_.history_click
          self.history_create = _default_panel_.history_create
        },
        set_content: function() {
            let self = window.modul['withings_heart']
            if (!($('#content_modul_page_withings_heart').length)) {
                let html = '<div id="content_modul_page_withings_heart" class="content_modul"></div>'
                $(".main_content").append(html)
                require([], function() {
                    window.api_call(url='sm/get_cards', data={'panel': 'heart'}).then(resp => {
                        if (resp.ok) {
                            ...
                            require(req, function(Packery){
                                for (var j=0; j<items.length; j++) {
                                    ...
                                    cards[type].create('withings_heart', items[j]['label'], j, item_data)
                                }
                                $('#content_modul_page_withings_heart').data('packery', new Packery( '#content_modul_page_withings_heart', {itemSelector: '.sh_card', gutter: 20}))
                            })
                            console.log(resp)
                        }
                    })
            ...
            $('#content_modul_page_withings_heart').show()
        },
        show: function () {
            let self = window.modul['withings_heart']
            window.modul['helper'].activate('withings_heart', self.label)
            self.set_content()
        },
        stop: function() {
    ...
    window.modul['withings_heart'].init()
});
